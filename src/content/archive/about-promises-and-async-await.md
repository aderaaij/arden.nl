...
description: "Course notes that got out of hand: promises from the ground up, chaining, error handling, Promise.all, and how async/await makes asynchronous code read like synchronous code."
date: 2018-01-20
---
...
ECMAScript 2015 (ES6) brought us the `Promise`, a native object which acts as a proxy (or temporary value) for a value that isn't known yet. Promises allow you to create handlers that deal with the eventual success or failure of an asynchronous action. In this post I'll tell you about all the ways to use promises, how to roll your own `Promise` functions, how to combine and chain promises and how to make the `Promise` api even better with `async / await`. But first we're going back to the absolute basics: Synchronous and Asynchronous code.
...
This doesn't look too bad. But what if we have another function with callback that needs to be called within our callback, and another one within that function? It's going to get complicated real quickly and it's what we commonly refer to as [callback hell](http://callbackhell.com/).
...
All the indenting makes the code very hard to read. There were ways around this, but that's not important anymore, because we've got the `Promise`!
...
In the script above we forgot `https://` so the browser is now looking for this api in my local files. If you run this, the error should say `Uncaught (in promise)`. This means that we haven't used the `catch` method we can tag on to the promise returned by the `fetch api`.
...
In the above example `mediaDevices.getUserMedia({ video: true })` triggers a prompt to ask user permission to get webcam access. When the user either accepts or rejects, the promise resolves or rejects. When we accept, we set the `mediaStream` as `srcObject` for our video element, load up the video and play it.

## Building your own promises

There are plenty of use cases when we'd want to make our own promises and here I'll show you how to do just that.
...
It takes in *one* function called the `executor` ☠️ with the arguments of `resolve` and `reject`. The idea being that whatever the promise returns either resolves and returns some kind of value, or results in an error.
...
In this example we've got two promises, `printThor` returns after `500ms`, `printQuotes` after `1000ms`. We feed them to `Promise.all` in an array, and call `.then`. This returns data when *both* promises are resolved. To make life easy we destructure both values right in the arrow function arguments.
...
If you remember that piece we wrote on callback hell:
...
At the core, Async / Await is built on top of promises.
...
In this case we take the exact same `Promise` we created in the previous code block, and call it in a function marked with `async`. Just by adding `await` in front of your function, your code will run synchronously and each `wait` function waits until the previous one has resolved. This API is even clearer than the `.then` method and there seems to be some performance gains as well. At the moment of writing `async / await` is supported by most browsers.
...
In the example above, the result of the first promise gets logged after 600ms the second after 1600ms and the third after 3000ms, as each `await` waits for the previous promise to resolve.
...
const catchErrors = (fn) => {
    ...
    return function() {
        // Which returns our async function, which is a promise on which we can call `.catch`
        return fn().catch((err) => {
...
- [Iteration Protocols - MDN web docs](https://developer.mozilla.org/nl/docs/Web/JavaScript/Reference/Iteration_protocols)
